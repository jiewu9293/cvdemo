...
morphology = cv2.morphologyEx(thresh,cv2.MORPH_CLOSE,kernal,iterations=3) # specify the img then the operation then the kernal, specify the iterations, if the effect is not obvious increase the number of iterations

#display the image after dilation
cv2.imshow('Eroded',morphology)
